var keyMapping = {
  // data-gtm-page - data-gtm-body
  page_title: "{{page title}}",   
  screen_id: "{{page id}}", 
  frame: "SPA", // "SPA" 고정값
  card_name: "{{카드 신청 단계 - 카드명}}",
  card_code: "{{카드 신청 단계 - 카드코드}}",
  card_new_or_exist: "{{카드 신청 단계 = 신규신청 구분}}",
  card_type: "{{카드 신청 단계 - 완편, 중편, 간편}}",
  fin_prod_name: "{{대출명 - 단기카드대출_일반, 단기카드대출_카드대금}}",
  fin_amount: "{{대출 - 신청 금액}}",
  revol_rate: "{{리볼빙 - 리볼빙 결제 비율}}",
  revol_term: "{{리볼빙 - 신청 기간}}",
  prod_funnel_name: "{{신청단계}}",
  
  // data-gtm-section - data-gtm-body
  label: "{{섹션 값}}",
  index: "{{섹션별 인덱스 값}}",
  is_popup: "{{True - 1 False - 미세팅}}",

  //data-gtm-click & data-gtm-visibility - data-gtm-body
  label: "{{클릭 텍스트}}",
  category_depth1: "{{클릭요소 상위 텍스트1}}",
  category_depth2: "{{클릭요소 상위 텍스트2}}",
  category_depth3: "{{클릭요소 상위 텍스트3}}",
  frame: "SPA", // "SPA" 고정값
  search_keyword: "{{검색 완료 페이지에서 인풋 키워드}}",
  search_type: "{{검색 타입}}",
  search_result: "{{검색 결과 메뉴 및 결과 갯수}}",
  card_name: "{{카드 신청 단계 - 카드명}}",
  card_code: "{{카드 신청 단계 - 카드코드}}",
  card_new_or_exist: "{{카드 신청 단계 = 신규신청 구분}}",
  card_type: "{{카드 신청 단계 - 완편, 중편, 간편}}",
  fin_prod_name: "{{대출명 - 단기카드대출_일반, 단기카드대출_카드대금}}",
  fin_amount: "{{대출 - 신청 금액}}",
  revol_rate: "{{리볼빙 - 리볼빙 결제 비율}}",
  revol_term: "{{리볼빙 - 신청 기간}}",
  prod_funnel_name: "{{신청단계}}",
  cts_name: "{{LADM 컨텐츠명 - 부담없이 환승에서 만나요}}",
  cts_id: "{{LADM 컨텐츠ID - LTS0000063451}}",
  cts_sub_id: "{{LADM 서브컨텐츠ID - 0000063451ABC}}",
  index: "{{컨텐츠 가로 인덱스 - 1 , 2, 3 ...}}",
};